...
### 匈牙利分配
## 🎯 示例：为什么 DETR 匹配阶段只看 Ground Truth 所在的类别（而不考虑其他类）

### 📦 模型预测结果（经过 softmax 后的分类概率）

| 预测编号 | 类别 A | 类别 B | 类别 C | 预测框坐标 `(cx, cy, w, h)` |
|----------|--------|--------|--------|-------------------------------|
...
我们使用：cost_class = 1 - p(predicted_class == GT_class)
...
> ✅ **注意**：我们完全不看预测对“C 类”的概率，因为 GT 中没有 C 类。

---

## 🔍 为什么不看 C 类？

- 因为每次计算代价时，是针对某一个 GT（它有已知类别）去选出最好的预测
- 与这个 GT 无关的类别（比如 C 类）不会影响这次匹配
- 多看反而干扰匹配结果，降低准确性
